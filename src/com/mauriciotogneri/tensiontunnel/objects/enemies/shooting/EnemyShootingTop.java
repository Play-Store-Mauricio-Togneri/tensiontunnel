...
public class EnemyShootingTop extends EnemyShooting
{
	...
	{
		super(new Rectangle(x - (EnemyShooting.ENEMY_WIDTH / 2f), Renderer.RESOLUTION_Y - Background.WALL_HEIGHT - EnemyShooting.ENEMY_WIDTH, EnemyShooting.ENEMY_WIDTH, EnemyShooting.ENEMY_WIDTH, EnemyShooting.COLOR), timeLimit);
	}
	
	@Override
	protected Beam getNewBeam(float beamSpeed)
	{
		return new BeamDown(this.rectangle.getX() + (this.rectangle.getWidth() / 2f), Renderer.RESOLUTION_Y - Background.WALL_HEIGHT - EnemyShooting.ENEMY_WIDTH - Beam.BEAM_HEIGHT, -beamSpeed);
	}
}