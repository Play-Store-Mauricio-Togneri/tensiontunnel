...
public class Renderer implements android.opengl.GLSurfaceView.Renderer
{
	...
	{
		return ((x / this.width) * Constants.Screen.RESOLUTION_X);
	}
	...
	private void processInput(float x, boolean pressed)
	{
		synchronized (this.inputLock)
		{
			if (pressed)
			{
				this.input.press(x, Constants.Screen.RESOLUTION_X);
			}
			else
			{
				this.input.release(x, Constants.Screen.RESOLUTION_X);
			}
		}
	}
	...
	public void onSurfaceChanged(GL10 screen, int width, int height)
	{
		...
		Constants.Screen.RESOLUTION_X = 100;
		Constants.Screen.RESOLUTION_Y = (int)(Constants.Screen.RESOLUTION_X / ((float)width / (float)height));

		Matrix.orthoM(this.projectionMatrix, 0, 0, Constants.Screen.RESOLUTION_X, 0, Constants.Screen.RESOLUTION_Y, -1f, 1f);
